...
{	
	return y * m_width * 3 +y*padding()+ 3 * x + channel;
}
uint8_t Bitmap::getPixelChannel(int x, int y, int channel) const
{
	if (x < 0 || x >= m_width || y<0 || y>=m_height)
		return m_buffer[
			getPixelId(clamp<int>(x, 0, int(m_width)-1),
				clamp<int>(y, 0, int(m_height) - 1), channel)];
